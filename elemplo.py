...
if __name__ == "__main__":
    OBJETO_TARGET = bpy.data.objects.get('Armature')
    FOLDER_BASE = r"C:/Users/windo/OneDrive/Documentos/ajedrez-buscaminas/renderizado/"
    NOMBRE = 'Rey'
    Acciones = [
        {
            'accion': 'static',
            'mode': 'estatico', 'end': 20, 'start': 1,
            'angle': [0, 180, 45, -45, 135, -135]
        },
        {
            'accion': 'run',
            'mode': 'animacion', 'end': 16, 'start': 1,
            'angle': [0, 180, 45, -45, 135, -135]
        },
        {
            'accion': 'atack',
            'mode': 'animacion', 'end': 19, 'start': 1,
            'angle': [0, 180, 45, -45, 135, -135]
        },
        {
            'accion': 'perder',
            'mode': 'animacion', 'end': 101, 'start': 1,
            'angle': [0, 180]
        }
    ]
    ...
    if OBJETO_TARGET is not None:
        ...
        bpy.data.objects['Camera'].location.y = -4
        bpy.data.objects['Camera'].rotation_euler.z = math.radians(0)
        ...
        bpy.data.objects['Camera'].location.y = 4
        bpy.data.objects['Camera'].rotation_euler.z = math.radians(180)
        for accion in Acciones:
            procesar_accion_render(
                NOMBRE+'Inverted', 
                accion['accion'], 
    ...
    # 1. Guarda los últimos cambios del archivo
    bpy.ops.wm.save_mainfile()

    # 2. Programa el apagado de Windows a los 120 segundos (2 minutos)
    os.system("shutdown /s /f /t 120")

    # 3. Cierra Blender de inmediato de forma limpia
